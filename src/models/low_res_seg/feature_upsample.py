...
class FeatureUpsampleSentinelNet(nn.Module):
    def __init__(self, model, keys, out_features=512, sizes=(1, 2, 3, 6)):
        super(FeatureUpsampleSentinelNet, self).__init__()
        self.keys = keys
        ...
        self.convp1 = nn.Sequential(
            nn.UpsamplingBilinear2d((self.input_size[0]*2, self.input_size[0]*2)),
        ...
        self.convp2 = nn.Sequential(
            ...
            nn.UpsamplingBilinear2d((self.input_size[0]*4, self.input_size[0]*4)),
            nn.BatchNorm2d(64),
            nn.PReLU()
        )
        self.convp3 = nn.Sequential(
            nn.Conv2d(64, 64, 3, padding=1),
            nn.UpsamplingBilinear2d((self.input_size[0]*8, self.input_size[0]*8)),
            nn.BatchNorm2d(64),
            nn.PReLU()
        )
...
def feature_upsample_net_34_s2_all():
    keys = ["pre_img10", "post_img10", "pre_img20", "post_img20"]
    in_channels = 4 + 4 + 6 + 6
    model = dilated_resnet34_flexible_input(in_channels)

    return FeatureUpsampleSentinelNet(model, keys)


def feature_upsample_net_34_s1_all():
...
def feature_upsample_net_34_s1s2():
    keys = ["pre_img10", "post_img10", "pre_img20", "post_img20",
...
def feature_upsample_net_50_s2_all():
    keys = ["pre_img10", "post_img10", "pre_img20", "post_img20"]
    in_channels = 4 + 4 + 6 + 6
    model = dilated_resnet50_flexible_input(in_channels)

    return FeatureUpsampleSentinelNet(model, keys, out_features=2048)


def feature_upsample_net_50_s1_all():
    keys = ["pre_sar", "post_sar", "multitemp_img",
            "pre_sar_intensity", "post_sar_intensity"]
    in_channels = 1 * 5
    model = dilated_resnet50_flexible_input(in_channels)
