...
def load_weights_sequential(target, source_state):
    ...
    for (k1, v1), (k2, v2) in zip(target.state_dict().items(),
                                  source_state.items()):
        new_dict[k1] = v2
...
class KeepDilatedResNet(nn.Module):
    def __init__(self, input_channels, block, layers):
        super(KeepDilatedResNet, self).__init__()
        self.inplanes = 64
        self.input_channels = input_channels

        self.conv1 = nn.Conv2d(input_channels, 64, kernel_size=7, stride=1,
                               padding=3,
                               bias=False)
...
def dilated_resnet18_keep_input(input_channels, pretrained=True):
    model = KeepDilatedResNet(input_channels, BasicBlock, [2, 2, 2, 2])
    if pretrained:
...
def dilated_resnet34_keep_input(input_channels, pretrained=True):
...
def dilated_resnet50_keep_input(input_channels, pretrained=True):
    model = KeepDilatedResNet(input_channels, Bottleneck, [3, 4, 6, 3])
    if pretrained:
...
def input_keep_res_net_34_s2_all():
...
def input_keep_res_net_34_s1_all():
